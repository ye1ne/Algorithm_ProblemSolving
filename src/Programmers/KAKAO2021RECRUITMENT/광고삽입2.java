...
public class 광고삽입2 {
    ...
    static public String solution(String play_time, String adv_time, String[] logs) {
        ...
        for(int[] tmp : list) {
            min = tmp[0];
            max = tmp[1];
            totalCountWatching[min-1] = nextTotal;
            for (int i = min; i <= max; i++) {
                if( i == min) {
                    totalCountWatching[i] = nextTotal + countWatching[i];
                }else{
                    totalCountWatching[i] = totalCountWatching[i - 1] + countWatching[i];
                }
            }
            nextTotal = totalCountWatching[max];
        }
        int[] arr3 = Arrays.copyOfRange(totalCountWatching,5457,5559);
        int[] arr4 = Arrays.copyOfRange(totalCountWatching,6807,6900);
        ...
        int[] arr1 = Arrays.copyOfRange(totalCountWatching,5458,5559);
        int[] arr2 = Arrays.copyOfRange(totalCountWatching,6808,6900);
